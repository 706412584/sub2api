  ...
  <div class="mb-4 flex items-center justify-between rounded-lg bg-primary-50 p-3 dark:bg-primary-900/20">
    ...
    <div class="flex gap-2">
      <template v-if="selectedIds.length > 0">
        ...
        <button @click="$emit('batch-scheduled-test')" class="btn btn-secondary btn-sm">{{ t('admin.accounts.bulkActions.batchScheduledTest') }}</button>
...
defineProps(['selectedIds']); defineEmits(['delete', 'edit-selected', 'edit-filtered', 'clear', 'select-page', 'toggle-schedulable', 'reset-status', 'refresh-token', 'batch-test', 'batch-scheduled-test']); const { t } = useI18n()
</script>
