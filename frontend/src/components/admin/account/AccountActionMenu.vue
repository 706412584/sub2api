      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="account.platform === 'kiro'" @click="$emit('kiro-details', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="infoCircle" size="sm" class="text-indigo-500" />
              {{ t('admin.accounts.kiroDetails.title') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'kiro-details', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
