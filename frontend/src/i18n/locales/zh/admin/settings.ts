export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        antigravityClientFingerprint: 'Antigravity 客户端指纹头',
        antigravityClientFingerprintHint: '开启后上游 v1internal 请求附带 x-client-name / x-client-version / x-machine-id / x-vscode-sessionid（每账号独立派生，重启换会话）。默认关闭；开启前建议先在单账号上灰度观察 401/403/429 基线。',
      ...
      grokReasoningVisibility: {
        ...
        modeEnforce: '强制排除',
        probeTtl: '探测缓存 TTL（秒）',
        probeTtlHint: '探测结果保持有效的时长；0=每次重探测；N=缓存 N 秒',
        quarantineSec: '探测失败冷却（秒）',
        quarantineSecHint: 'enforce 失败后的冷却：0=仅本轮排除；N=临时不可调度 N 秒（默认 120）',
      ...
      grokCliIdentity: {
        ...
        sourceDefault: '编译默认',
        latest: 'npm latest',
        latestCheckedAt: '检测时间',
        updateAvailable: '有可用更新',
        versionInput: '手动覆盖版本',
        versionInputHint: '需为规范 semver，且 ≥ 编译默认；留空并保存可清除覆盖',
        check: '检测更新',
        checking: '检测中…',
        applyLatest: '一键应用 latest',
        applying: '应用中…',
        restoreDefault: '恢复编译默认',
        restoring: '恢复中…',
        save: '保存覆盖',
        saving: '保存中…',
        none: '无',
        checkOk: '已检测 npm latest',
        checkFailed: '检测 npm latest 失败',
        applyOk: '已应用 npm latest',
        applyFailed: '应用 npm latest 失败',
        restoreOk: '已恢复编译默认',
        restoreFailed: '恢复编译默认失败',
        saved: 'Grok CLI 身份版本已保存',
        saveFailed: '保存 Grok CLI 身份版本失败'
      },
      accountPoolProbe: {
        title: '号池异步自动探测',
