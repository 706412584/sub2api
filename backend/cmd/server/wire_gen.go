...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	egressProxyResolver := repository.NewEgressProxyResolver(proxyRepository)
	httpUpstream := repository.NewHTTPUpstreamWithEgress(configConfig, egressProxyResolver)
	...
	grokOAuthClient := repository.NewGrokOAuthClient(egressProxyResolver)
	...
	grokConsoleDPoPProvider := service.ProvideGrokConsoleDPoPProvider(grokSessionCredentialService, httpUpstream, proxyRepository)
	...
	proxyExitInfoProber := repository.NewProxyExitInfoProberWithEgress(configConfig, egressProxyResolver)
	...
	kiroGatewayService := service.NewKiroGatewayService(httpUpstream, tlsFingerprintProfileService, rateLimitService, settingService)
	codebuddyGatewayService := service.ProvideCodebuddyGatewayService(httpUpstream, tlsFingerprintProfileService, settingService, configConfig)
	usageCache := service.NewUsageCache()
	accountUsageService := service.ProvideAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, grokQuotaFetcher, grokQuotaService, openAIQuotaService, kiroGatewayService, codebuddyGatewayService, usageCache, identityCache, tlsFingerprintProfileService, openAIGatewayService, grokConsoleDPoPProvider)
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, proxyRepository, geminiTokenProvider, claudeTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, openAIGatewayService, settingService, grokConsoleDPoPProvider, pluginManager)
	...
	kiroBuilderIDDeviceFlowService := service.ProvideKiroBuilderIDDeviceFlowService()
	kiroOAuthHandler := admin.NewKiroOAuthHandler(adminService, kiroBuilderIDDeviceFlowService, accountTestService, kiroGatewayService)
	codebuddyLoginService := service.ProvideCodebuddyLoginService()
	codebuddyOAuthHandler := admin.NewCodebuddyOAuthHandler(adminService, codebuddyLoginService, accountTestService, codebuddyGatewayService)
	...
	updateService := service.ProvideUpdateService(updateCache, gitHubReleaseClient, serviceBuildInfo, settingRepository)
	...
	proxySubscriptionRepository := repository.NewProxySubscriptionRepository(client)
	mihomoEngine := service.ProvideMihomoEngine(configConfig)
	...
	imBotRepository := repository.NewIMBotRepository(client)
	hub := im.ProvideIMHub(configConfig, imBotRepository, apiKeyRepository, secretEncryptor, redisClient)
	imBotService := im.ProvideIMBotService(imBotRepository, apiKeyRepository, secretEncryptor, hub, configConfig)
	hubRef := im.ProvideHubRef(hub)
	imBotHandler := admin.NewIMBotHandler(imBotService, hubRef)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, httpUpstream, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, kiroOAuthHandler, codebuddyOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, proxySubscriptionHandler, dynamicProxyPoolHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, imBotHandler, upstreamBillingProbeService, gatewayService, ollamaCloudUsageService, grokSessionCredentialService, openCodeGoUsageService)
	...
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, kiroGatewayService, codebuddyGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, coordinator)
	...
	proxySubscriptionRunner := service.ProvideProxySubscriptionRunner(proxySubscriptionService, configConfig, leaderLockCache, db)
	dynamicProxyPoolRunner := service.ProvideDynamicProxyPoolRunner(dynamicProxyPoolService, leaderLockCache, db)
	channelMonitorV2Aggregator := service.ProvideChannelMonitorV2Aggregator(channelMonitorV2Repository, db, settingService)
	userPlatformQuotaUsageFlusher := service.ProvideUserPlatformQuotaUsageFlusher(configConfig, billingCache, serviceUserPlatformQuotaRepository, timingWheelService)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, cnProviderBalanceCheckService, openAICodexVersionSyncService, claudeCodeVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, accountPoolProbeRunner, codebuddyMaintenanceRunner, backupService, paymentOrderExpiryService, channelMonitorRunner, proxySubscriptionRunner, dynamicProxyPoolRunner, channelMonitorV2Aggregator, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, ollamaCloudUsageService, openCodeGoUsageService, auditLogService, openAIQuotaAutoResetService, promptService, pluginManager, hub)
