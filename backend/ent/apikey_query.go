...
// QueryImBots chains the current query on the "im_bots" edge.
func (_q *APIKeyQuery) QueryImBots() *IMBotQuery {
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withImBots:    _q.withImBots.Clone(),
...
// WithImBots tells the query-builder to eager-load the nodes that are connected to
// the "im_bots" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithImBots(opts ...func(*IMBotQuery)) *APIKeyQuery {
	query := (&IMBotClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withImBots; query != nil {
		if err := _q.loadImBots(ctx, query, nodes,
			func(n *APIKey) { n.Edges.ImBots = []*IMBot{} },
			func(n *APIKey, e *IMBot) { n.Edges.ImBots = append(n.Edges.ImBots, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadImBots(ctx context.Context, query *IMBotQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *IMBot)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(imbot.FieldAPIKeyID)
	}
	query.Where(predicate.IMBot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.ImBotsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
