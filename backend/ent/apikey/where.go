...
// HasImBots applies the HasEdge predicate on the "im_bots" edge.
func HasImBots() predicate.APIKey {
	return predicate.APIKey(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasImBotsWith applies the HasEdge predicate on the "im_bots" edge with a given conditions (other predicates).
func HasImBotsWith(preds ...predicate.IMBot) predicate.APIKey {
	return predicate.APIKey(func(s *sql.Selector) {
		step := newImBotsStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
