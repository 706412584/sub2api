...
type APIKeyEdges struct {
	...
	// ImBots holds the value of the im_bots edge.
	ImBots []*IMBot `json:"im_bots,omitempty"`
...
// ImBotsOrErr returns the ImBots value or an error if the edge
// was not loaded in eager-loading.
func (e APIKeyEdges) ImBotsOrErr() ([]*IMBot, error) {
	if e.loadedTypes[3] {
		return e.ImBots, nil
	}
	return nil, &NotLoadedError{edge: "im_bots"}
}
...
// QueryImBots queries the "im_bots" edge of the APIKey entity.
func (_m *APIKey) QueryImBots() *IMBotQuery {
	return NewAPIKeyClient(_m.config).QueryImBots(_m)
}
