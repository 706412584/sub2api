...
func refreshKiroExternalIDPToken(ctx context.Context, _ *http.Client, account *Account, refreshToken string) (*KiroTokenInfo, error) {
	clientID := strings.TrimSpace(account.GetCredential("client_id"))
	...
	secureClient, err := newKiroExternalIDPHTTPClient(tokenEndpoint)
	if err != nil {
		return nil, err
	}
	form := url.Values{}
...
func newKiroExternalIDPHTTPClient(rawEndpoint string) (*http.Client, error) {
	parsed, err := validateKiroExternalIDPTokenEndpoint(rawEndpoint)
	if err != nil {
		return nil, err
	}
	...
	transport.DialContext = func(ctx context.Context, network, address string) (net.Conn, error) {
		...
		if !strings.EqualFold(host, parsed.Hostname()) {
			return nil, fmt.Errorf("external IdP redirects are not allowed")
		}
		if port != "443" {
			return nil, fmt.Errorf("external IdP token_endpoint must use the default https port")
		}
		...
		for _, ip := range ips {
			if isPrivateKiroEndpointIP(ip) {
				return nil, fmt.Errorf("external IdP token_endpoint must not resolve to a private address")
			}
		}
		if len(ips) == 0 {
			return nil, fmt.Errorf("external IdP token_endpoint host has no addresses")
		}
...
func validateKiroExternalIDPTokenEndpoint(rawEndpoint string) (*url.URL, error) {
	parsed, err := url.Parse(rawEndpoint)
	if err != nil {
		return nil, fmt.Errorf("invalid external IdP token_endpoint")
	}
	if parsed.Scheme != "https" || parsed.User != nil || parsed.Host == "" {
		return nil, fmt.Errorf("external IdP token_endpoint must be an https URL")
	}
	if parsed.Port() != "" && parsed.Port() != "443" {
		return nil, fmt.Errorf("external IdP token_endpoint must use the default https port")
	}
	if strings.EqualFold(parsed.Hostname(), "localhost") {
		return nil, fmt.Errorf("external IdP token_endpoint must not use localhost")
	}
	if ip := net.ParseIP(parsed.Hostname()); ip != nil && isPrivateKiroEndpointIP(ip) {
		return nil, fmt.Errorf("external IdP token_endpoint must not resolve to a private address")
	}
	return parsed, nil
}

func isPrivateKiroEndpointIP(ip net.IP) bool {
