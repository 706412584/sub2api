// response.go 处理上游 SSE 流：聚合成单个 OpenAI 响应，或逐帧规范化透传。
package codebuddy
...
// ErrEmptyStream 上游返回 200 但流内无任何有效数据事件。
var ErrEmptyStream = fmt.Errorf("codebuddy: upstream stream contained no valid data events")
	...
	for {
		...
		if strings.HasPrefix(line, "data: ") {
			...
			if json.Unmarshal([]byte(payload), &chunk) == nil {
				// 有效事件计数：仅 JSON 解析成功的数据帧计入（解析失败静默跳过）。
				validEvents++
...
// FrameWriter 是 Stream 写出目标的最小抽象（*gin.ResponseWriter / http.ResponseWriter 均满足）。
...
//
// 返回 (有效帧数, 错误)：有效帧数为 0 时返回 ErrEmptyStream，
// 此时仍已写出一个 error 帧与 [DONE]，保证客户端能正常收尾。
func Stream(w FrameWriter, r io.Reader) error {
	br := bufio.NewReaderSize(r, 64*1024)
	validFrames := 0
readLoop:
	for {
		...
		case strings.HasPrefix(trimmed, "data: "):
			n, werr := writeFrame(w, strings.TrimPrefix(trimmed, "data: "))
			validFrames += n
			if werr != nil {
				return werr
			}
	...
	// 再补 [DONE] 保证客户端能正常收尾。
	if validFrames == 0 {
		_ = writeRaw(w, `{"error":{"message":"empty upstream stream","type":"upstream_error"}}`)
	}
...
func writeFrame(w FrameWriter, payload string) (int, error) {
...
// 不能被白名单剥掉。
func writeRaw(w FrameWriter, payload string) error {
	if _, werr := io.WriteString(w, "data: "+payload+"\n\n"); werr != nil {
		return werr
	}
	w.Flush()
...
// SetSSEHeaders 设置 SSE 响应头（调用方在写状态码前调用）。
func SetSSEHeaders(h map[string][]string) {
