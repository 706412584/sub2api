...
- Fixes admin Kiro account testing so it no longer probes the Anthropic endpoint with a Kiro token. The test path now uses Kiro's native request shape:
  ```http
...
- Redacts additional credential secrets (`client_secret`, `external_idp`) from account API responses and adds tests for import parsing, native Kiro test request headers, external IdP refresh, quota SQL/platform allowlists, and credential redaction.
