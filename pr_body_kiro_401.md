...
- Fixes admin Kiro account testing so it uses Kiro's native request shape instead of probing Anthropic with a Kiro token, and retries once with a freshly refreshed Kiro token when upstream returns 401/403 auth errors:
  ```http
...
- Redacts additional credential secrets (`client_secret`, `external_idp`) from account API responses and adds tests for import parsing, native Kiro test request headers, auth-failure refresh retry, external IdP refresh, quota SQL/platform allowlists, and credential redaction.
